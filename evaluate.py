...
class Evaluate:
    ...
    def evaluate(self):
        ...
        for element in self.postfix_expr:
            ...
            if element.type == "ID" and element.value not in ['div', 'mod','sin', 'cos', 'tan', 'cot', 'arcsin', 'arccos', 'arctan', 'arccot', 'log', 'sqrt', 'sqr', 'exp']:
                ...
                if len(operand_stack) < 2:
                    raise Exception(f"len of the stack is less than required for a double-input operation: {operand_stack}")
                ...
                if len(operand_stack) < 1:
                    raise Exception(f"len of the stack is less than required for a single-input operation: {operand_stack}")
                top_element = operand_stack.pop()
        ...
        if len(operand_stack) != 1:
            raise Exception("The postfix expression is invalid. Remaining stack:", operand_stack)
